...
# The samtools version should generally be the most recent release
SAMTOOLS_VERSION=${BATON_VERSTION:-"1.15.1"}
...
conda install -y -c https://dnap.cog.sanger.ac.uk/npg/conda/devel/generic/ \
    -n ${CONDA_ENVIRONMENT} "samtools=${SAMTOOLS_VERSION}"
conda install -y -c https://dnap.cog.sanger.ac.uk/npg/conda/devel/generic/ \
    -n ${CONDA_ENVIRONMENT} libhts-plugins
